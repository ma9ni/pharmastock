...
class ProductService {
  ...
  static Future<List<MaterialModel>> getAllProducts() async {
    if (ApiConfig.useMock) {
      ...
      return MockService.getProducts().map(MaterialModel.fromJson).toList();
    }
    try {
  ...
  static Future<void> updateProduct(
    ...
    if (ApiConfig.useMock) {
      ...
      MockService.updateProduct(id, body);
      return;
    }
    if (!ConnectivityService().isOnline.value) {
      await OfflineService.queueOperation(
  ...
  static Future<List<Map<String, dynamic>>> getBatches(String productId) async {
    if (ApiConfig.useMock) {
      ...
      if (batches is List) {
        return batches.map((b) => Map<String, dynamic>.from(b as Map)).toList();
      }
      return [];
    }
    try {
  ...
  static Future<Map<String, dynamic>> receiveStock(
    ...
    if (ApiConfig.useMock) {
      await Future.delayed(const Duration(milliseconds: 300));
      return MockService.receiveStock(productId, quantity, expiryDate);
    }
    final body = {
      'quantity': quantity,
  ...
  static Future<Map<String, dynamic>> getFefoPlan(
    ...
    if (ApiConfig.useMock) {
      await Future.delayed(const Duration(milliseconds: 200));
      return MockService.getFefoPlan(productId, quantity);
    }
    try {
      final response =
