...
class OrderService {
  ...
  static Future<Map<String, dynamic>?> dispatchFefo(Map<String, dynamic> body) async {
    if (ApiConfig.useMock) {
      ...
      final result = MockService.dispatchFefo(body);
      final order = OrderModel.fromJson(result['order'] as Map<String, dynamic>);
      _orders.insert(0, order);
      changes.value++;
      return result;
    }
    ...
    try {
      ...
      final msg = decoded is Map
          ? (decoded['message'] ?? 'Dispatch failed (${response.statusCode})').toString()
          : 'Dispatch failed';
      throw Exception(msg);
    } catch (e) {
      rethrow;
  ...
  static Future<bool> checkInvoiceExists(String invoiceNumber) async {
    ...
    try {
      final response = await http
          .get(
            Uri.parse('$_baseUrl/invoices/exists/$invoiceNumber'),
            headers: AuthService.authHeaders,
          )
          .timeout(const Duration(seconds: 45));
      if (response.statusCode == 200) {
  ...
  static Future<void> _fetchOrders() async {
    if (ApiConfig.useMock) {
      await Future.delayed(const Duration(milliseconds: 400));
      final mockOrders = MockService.getOrders();
      _orders.clear();
