...
class ContactService {
  ...
  static Future<List<Contact>> getContacts({String? type}) async {
    if (ApiConfig.useMock) {
      ...
      return MockService.getContacts(type: type)
          .map((m) => Contact.fromJson(m))
          .toList();
    }
    var url = _baseUrl;
  ...
  static Future<Contact> createContact({
    ...
    if (ApiConfig.useMock) {
      ...
      return Contact.fromJson(MockService.addContact({
        'name': name,
        'type': type,
        'phone': phone,
        'notes': notes,
  ...
  static Future<Contact> updateContact(
    ...
    if (ApiConfig.useMock) {
      await Future.delayed(const Duration(milliseconds: 200));
      return Contact.fromJson(MockService.updateContact(id, {
        'name': name,
        'type': type,
        'phone': phone,
        'notes': notes,
